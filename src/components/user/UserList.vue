...
export default {
  ...
  methods: {
    async getUsers() {
      this.loading = true
      let me = this
      this.users.all = await userApi.getUsers({include_disabled: this.include_disabled})
        .then((res) => res)
        .catch(error => {
      ...
      this.users.sorted = this.sortUsers(this.users.all)
      this.pagination.totalItems = this.users.all.length
    },
    userStyle (user) {
      return userApi.userStyle(user)
    },
    sortUsers(users) {
      ...
      if (this.search) {
        users = this.filterUsers(users, this.search);
      }
      if (rowsPerPage > 0) {
    ...
    updateRowsPerPage() {
      ...
      localStorage.setItem(this.storagePrefix + "userListRowsPerPage", rowsPref);
    },
    completeAction() {
      this.updatePerson();
      this.users.selectedFormatted = "";
      this.users.selected = [];
    ...
    openDialog() {
      this.dialog = true;
      this.users.selectedFormatted = Array.from(
        this.users.selected,
        u => u.full_name)
          .toString()
          .replace(/,/g, ", ");
  ...
  computed: {
    ...
    isActionSelectActive() {
      return this.users.selected.length === 0;
    }
  },
  mounted() {
    this.getRowsPerPage();
    this.getUsers()
      .then(res => {
        this.$set(this.users, "all", res);
        this.$set(this.users, "sorted", this.sortUsers(res));
        this.$set(this.pagination, "totalItems", res.length);
      })
      ...
      <v-flex xs12>
        <v-card>
          ...
          <v-data-table
            ...
            :items="users.sorted"
            item-key="username"
            :total-items="this.users.all.length"
            :loading="loading"
            :pagination.sync="pagination"
            :rows-per-page-items="usersPerPageItems"
